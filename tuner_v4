...
void loop()
{
    ...
    unsigned long threshold = analogRead(A1) * (long) SAMPLES;
    //Serial.println(String(threshold) + ": threshold value.");
    ...
    Serial.println(notes[midi_num%12] + " (" + cents + "cents)");     //Print out the most dominant frequency.
    //Serial.println(String(power) + ": power value.");
    //Serial.println(String(cents) + ": cents value.");
    if (power >= threshold)
    {
      //Serial.println("Power > Thresh");
      setLed(cents);
    }
    delay(500);
}
